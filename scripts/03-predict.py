...
def test(path_to_images, path_output_maps, model_to_test=None):
    #list_img_files = [k.split('\\')[-1].split('.')[0] for k in glob.glob(os.path.join(path_to_images, '*'))] # windows
    list_img_files = [k.split('/')[-1].split('.')[0] for k in glob.glob(os.path.join(path_to_images, '*'))] # linux
    # Load Data
    list_img_files.sort()
